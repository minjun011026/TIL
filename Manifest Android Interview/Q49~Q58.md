...
3. ViewModel에서 PagingData를 관찰하고 RecyclerView에서 렌더링하기 위해 PagingDataAdapter에 전달한다.

## 실전 질문
Q) Paging 라이브러리는 데이터 로딩 중 오류를 어떻게 처리하며, 페이지네이션된 데이터 흐름에서 오류 처리 및 재시도 메커니즘을 구현하기 위해 사용해 본 전략은 무엇인가요?   
A) Paging 라이브러리는 데이터 로딩 중 오류가 발생하면 `LoadingResult.Error`로 오류를 노출하고 이를 `PagingDataAdapter.loadStateFlow`로 UI에서 상태를 관찰할 수 있다. 이때 시도할 수 있는 오류 처리 및 재시도 전략으로는 `LoadStateAdapter`를 상속받은 별도의 어댑터를 만들어 에러 발생시 재시도 버튼을 표시하거나 `retry()`로 실패한 요청부터 다시 재시도하는 방식을 사용할 수 있다.    
보다 사용자 경험을 개선하고자 한다면 RemoteMediator와 로컬DB를 연동하여 네트워크 에러 시에 로컬에 로딩된 데이터를 표시하여 끊기지 않는 경험을 제공할 수 있다.

# Q58 Baseline Profile은 앱의 성능에 어떤 이점을 가져다주나요?
BaselineProfiles 는 앱 시작 시간과 런타임 실행을 최적화하기 위한 안드로이드 앱의 성능 향상을 위한 플러그인이다.   
+ 미리 컴파일된 코드 정보를 제공함으로써 코드 해석 및 JIT 컴파일 단계를 우회하고 더 빠른 앱 실행을 가능하게 한다.

| **Baseline Profiles 작동 방식**   
1. 중요한 코드 경로 정의 : 주요 실행 결로를 프로파일링하거나 앱 시작 순간부터 가장 일반적인 사용자의 앱 사용 패턴을 기반으로 성능에 중요한 메서드 및 클래스를 사전 정의 가능
2. 프로파일 생성 : 프로파일은 Jetpack Macrobenchmark 라이브러리와 같은 도구를 사용하여 생성된다. 이를 통해 앱 동작을 기록하고 테스트하여 중요한 코드 경로 식별 가능
3. 프로파일 전파 : 생성된 Baseline Profile은 APK 또는 AAB와 함께 번들로 제공되어 최종 사용자에게 전파되어 배포된다.
4. 설치 중 최적화 : 앱이 사용자 기기에 설치될 때 ART는 프로파일을 사용하여 미리 정의해두었던 메서드 및 클래스를 네이티브 코드로 미리 컴파일한다.
...
A) Baseline Profiles를 활용하면 핵심 코드 경로를 기록한 프로파일을 ART에 제공하여 설치 시점에 주요 코드를 AOT(Ahead-Of-Time) 하도록 유도하여 Interpret/JIT 단계를 우회함으로서 성능을 개선한다.    
JIT 컴파일과 비교했을 때 JIT 컴파일은 실행 후에 최적화되어 초기 실행 지연과 런타임 CPU 부하라는 문제점이 있으나 이 방식을 통해 JIT 컴파일을 할 필요없이 처음부터 최적화가 적용되는 성능적 이점이 있다.
